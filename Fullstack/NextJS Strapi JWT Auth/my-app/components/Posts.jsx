...
import { faHeart, faEllipsisVertical, faTrash, faLink, faTriangleExclamation, faXmark } from '@fortawesome/free-solid-svg-icons'
...
export default function Posts({data: {postsApi, token, currentUser}}) {
    ...
    const filterButton = [{text: "Suggest", name: "suggest"}, {text: "Following", name: "following"}]


    async function filterHandler(e){
        if(e.target.name === "suggest"){
    ...
    <>
        ...
        <div className="flex justify-around border-b border-border-primary pt-2 pb-4 max-w-[390px] mx-auto">
            {filterButton.map(button => {
                return <button key={button.name} name={button.name} onClick={filterHandler} className={`px-4 relative after:absolute after:h-[3px] after:rounded-full after:bg-blue-400 after:-bottom-4 after:left-0 ${button.name === "suggest" ? "after:right-0" : ""}`}>{button.text}</button>
            })}
        </div>
        )}
        {!posts.length ? (
            currentPath !== "/" ? (
                ...
                <div className="text-center border border-dashed border-border-secondary w-max mx-auto px-4 py-2 rounded text-sm text-text-third mt-44">No posts yet : /</div>
            )
        ): (
        posts.map((post, i) => {
            return (
                            ...
                            <div className="text-right flex-1">
                                ...
                                <div id={post.id} ref={element => infoPost.current[i] = element} className="fixed -bottom-full transition-all z-50 bg-bg-primary duration-300 border border-border-primary max-w-[400px] sm:max-w-[500px] left-0 right-0 mx-auto overflow-auto h-[20%] rounded-t-xl">
                                    <div className="flex text-center items-center justify-evenly h-full relative">
                                        ...
                                        <div className="cursor-pointer hover:-translate-y-1 transition-all">
                                            <FontAwesomeIcon icon={faLink} size="xl" />
                                            <p className="text-text-third text-sm mt-1">Link</p>
                                        </div>
                                        <div className="cursor-pointer hover:-translate-y-1 transition-all">
                                            <FontAwesomeIcon icon={faBookmark} size="xl" />
                                            <p className="text-text-third text-sm mt-1">Save</p>
                                        </div>
                                    </div>
                                </div>
